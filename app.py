...
st.markdown("""
    <style>
        ...
        /* Header Utama dengan Gradasi Hijau Daun */
        .main-header {
            background: linear-gradient(135deg, #133820 0%, #2A6F37 100%);
            padding: 20px;
            border-radius: 12px;
            color: white;
            box-shadow: 0 4px 12px rgba(19, 56, 32, 0.15);
            margin-bottom: 20px;
            text-align: center;
        }

        /* Kartu Metrik Berwarna-warni yang Estetik */
        .metric-card-1 {
        ...
        /* Optimasi Tampilan Mobile / HP agar nyaman dibaca */
        @media screen and (max-width: 768px) {
            ...
            /* Menyesuaikan ukuran padding di HP */
            .block-container {
                padding-left: 1rem;
                padding-right: 1rem;
            }
...
# Inisialisasi & Migrasi Otomatis Database SQLite
def init_db():
...
# --- HEADER APLIKASI SIDAHTRA ---
st.markdown("""
    <div class='main-header'>
        <h1 style='margin:0;'>🌱 SIDAHTRA</h1>
        <p style='margin:4px 0 0 0; opacity: 0.9;'>Sistem Data Hibah Alsintan Terpadu</p>
    </div>
""", unsafe_allow_html=True)

# --- NAVIGASI ATAS (TOP TABS) ---
menu = st.tabs([
    "📊 Dashboard", 
    "📥 Input Baru", 
    "📋 Rekap", 
...
with menu[0]:
    ...
    st.markdown("<p style='color: #4A6B52;'>Statistik cepat data hibah alsintan yang tercatat di sistem.</p>", unsafe_allow_html=True)
    st.markdown("---")
    
    # Grid Kartu Metrik Berwarna
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f"""
    ...
    if not df_global.empty:
        ...
        df_latest.index.name = "No"
        st.dataframe(df_latest[["asal_usul", "tahun_hibah", "jenis_barang", "kelompok", "kecamatan", "jumlah"]], use_container_width=True)
    else:
...
with menu[1]:
    ...
    with st.form("form_sidahtra", clear_on_submit=True):
        ...
        jenis_barang = st.text_input("Nama/Jenis Alsintan")
        ...
        st.markdown("---")
        foto_file = st.file_uploader("Upload Foto Barang / Lokasi", type=["jpg", "png", "jpeg"])
...
with menu[2]:
    st.subheader("📋 Tabel Rekapitulasi SIDAHTRA")
    ...
    if not df.empty:
        ...
        st.info("💡 Di HP, Anda bisa menggeser tabel ke kiri/kanan untuk melihat kolom lainnya.")
...
with menu[4]:
    ...
    conn = sqlite3.connect("sidahtra_pertanian.db")
    df_db = pd.read_sql_query("SELECT id, kelompok, jenis_barang, kecamatan FROM hibah", conn)
    conn.close()
        ...
        if id_pilih:
            ...
            if data_row:
                with st.form("form_edit_berkas"):
                    ...
                    st.markdown("---")
                    new_foto = st.file_uploader("Ganti Foto Baru", type=["jpg", "png", "jpeg"])
